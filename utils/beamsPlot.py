...
def get_tiling_plot(meta):
    ...
    # Add known pulsar
    q = QueryATNF(params=['JNAME','RAJ','DECJ'],circular_boundary=(boresight_ra,boresight_dec,incoherent_beam_radius*1.1))
    ...
    for i in range(len(vals)):
        ...
        #if beam_no=='301':
        #    ellipse = Ellipse(xy=(pixel_beam_ra, pixel_beam_dec), width=beam_width, height=beam_height,angle = beam_angle, edgecolor='green', fc='green', lw=1.5,label='Best detection beam')
        #else:
...
#    circle_parkes = Circle((psr_pixel_ra,psr_pixel_dec), 0.233333333, color='purple',linestyle='--',linewidth=1.5,fill=False,label='Parkes beam')
