...
def calculate_ellipse_coefficients(angle, x_sigma, y_sigma):
    ...
    a = (cos_angle * cos_angle) / (2 * x_sigma_sq) + \
        (sin_angle * sin_angle) / (2 * y_sigma_sq)
    b = (-math.sin(2 * angle)) / (4 * x_sigma_sq) + \
        (math.sin(2 * angle)) / (4 * y_sigma_sq)
    c = (sin_angle * sin_angle) / (2 * x_sigma_sq) + \
        (cos_angle * cos_angle) / (2 * y_sigma_sq)
...
def get_beam_intersection(b1_coords, b2_coords, angle, x_sigma, y_sigma):
    ...
    if d > 700:  # exp(700) is approximately 1e304, close to the limit for 64-bit floats
        # Return 0 for very large d (beams are effectively non-overlapping)
        return 0.0
    else:
        return math.exp(d)
...
def get_neighbour_beams(opts, df):
    ...
    for i, row in df.iterrows():
        ...
        angle = np.radians(beam_shape['angle']) - \
            np.pi  # Convert angle to radians
        ...
        for beam_coord in filtered_beam_coords:
            intersection_value = get_beam_intersection(
                ref_coords, beam_coord, angle, x_sigma, y_sigma)
        ...
        neighbour_beam_list = [filtered_indices[i]
                               for i in sorted_indices[:6]]  # Select the 6 closest beams

        # Add neighboring beams to the DataFrame
        for beam_num in neighbour_beam_list:
            filterbank_path = os.path.dirname(
                row['filterbank_path']) + '/' + 'cfbf00{:03d}'.format(int(beam_num))
            neighbour_set.loc[len(neighbour_set)] = [
                filterbank_path, opts.username, opts.survey_name + ' T1_CAND n']
...
def write_t1_t2_beams(opts):
    ...
    classified_files = [c for c in glob.glob("{}/*{}*.csv".format(
        opts.main_dir, opts.tag)) if 'autosave' not in c if 'keep' not in c if 'full' not in c]
    # all_candidate_csvs = glob.glob("{}/**/candidates.csv".format(opts.main_dir))
    all_candidate_csvs = glob.glob("{}/candidates.csv".format(opts.main_dir))

    if len(classified_files) != len(all_candidate_csvs):
        raise Exception(
            "The total number of files with input tag does not match the total candidate.csv files! Please check for multiple redundant files")
    ...
    t1_t2_df = (df[(df['classification'] == 'T1_CAND')
                | (df['classification'] == 'T2_CAND')])

    if t1_t2_df.empty:
        log.info(
            'No T1/T2 files in {}. Returning empty T1/T2 list'.format(os.path.dirname(classified_file)))
        return t1_t2_all

    else:
        candidate_meta_df = pd.read_csv(candidate_csv)
        t1_t2_beams_df = candidate_meta_df[candidate_meta_df['png_path'].isin(
            t1_t2_df['png'])][['filterbank_path', 'png_path']].reset_index(drop=True)
        log.info(
            "Number of T1/T2 cands found: {}".format(t1_t2_beams_df.drop_duplicates().shape[0]))
        t1_t2_beams_df['filterbank_path'] = t1_t2_beams_df['filterbank_path'].apply(
            lambda x: x.replace(x, os.path.dirname(x)))
        t1_t2_beams_df = t1_t2_beams_df.assign(username=opts.username)
        t1_t2_beams_df_renamed = t1_t2_beams_df.rename(
            {'png_path': 'png'}, axis=1)
        merged_df = pd.merge(t1_t2_beams_df_renamed, t1_t2_df, on='png')
        merged_df = merged_df.assign(
            reason=opts.survey_name + ' ' + merged_df['classification'])
        t1_t2_all = pd.concat(
            [t1_t2_all, merged_df[['filterbank_path', 'username', 'reason']]], ignore_index=True)
        ...
        if opts.keep_neighbours == 1:
            if t1_t2_df['classification'].str.contains('T1_CAND').sum():
                log.info(
                    "T1 candidate(s) found. Neighbouring beams will be added by default")
                t1_df = df[df['classification'] == 'T1_CAND']
                t1_beams_df = candidate_meta_df[candidate_meta_df['png_path'].isin(
                    t1_df['png'])][['filterbank_path', 'beam_name', 'metafile_path']]
                t1_beams_df['filterbank_path'] = t1_beams_df['filterbank_path'].apply(
                    lambda x: x.replace(x, os.path.dirname(x)))
                t1_neighbour_set_df = get_neighbour_beams(opts, t1_beams_df)
                t1_t2_all = pd.concat(
                    [t1_t2_all, t1_neighbour_set_df], ignore_index=True)
            else:
...
def write_known_pulsar_beams(opts):
    ...
    if len(classified_files) != len(all_candidate_csvs):
        raise Exception(
            "The total number of classification files does not match the total candidate.csv files! Please check for multiple redundant files")
    ...
    for i, classified_file in enumerate(classified_files):
        ...
        if kp_df.empty:
            log.info('No known pulsar files in {}'.format(
                os.path.dirname(classified_file)))
            continue
        else:
            candidate_meta_df = pd.read_csv(all_candidate_csvs[i])
            kp_beams_df = candidate_meta_df[candidate_meta_df['png_path'].isin(kp_df['png'])][[
                'filterbank_path']]
            kp_beams_df['filterbank_path'] = kp_beams_df['filterbank_path'].apply(
                lambda x: x.replace(x, os.path.dirname(x)))

            kp_beams_df = kp_beams_df.assign(username=opts.username)

            kp_beams_df['reason'] = kp_df[kp_df['png'].isin(
                candidate_meta_df['png_path'])]['classification'].to_list()
            kp_beams_df['reason'] = opts.survey_name + \
                ' ' + kp_beams_df['reason'].astype(str)

            log.info("Number of known pulsar cands found: {}".format(
                kp_beams_df.drop_duplicates().shape[0]))
...
def write_out_second_revision_tar_file(opts):
    ...
    second_round_path = opts.output_dir + \
        '/{}_{}_round1'.format(os.path.basename(
            os.path.normpath(opts.main_dir)), opts.tag)
    try:
    ...
    for i, classified_file in enumerate(classified_files):
        ...
        if t1_t2_df.empty:
            ...
            t1_t2_meta_df = candidate_meta_df[candidate_meta_df['png_path'].isin(
                t1_t2_df['png'])]
            t1_t2_classified_df = t1_t2_df[t1_t2_df['png'].isin(
                candidate_meta_df['png_path'])]
            # Copy over metafile and png files to respective directories
            log.info("Copying over meta and png files to {}".format(
                second_round_path))
            ...
            t1_t2_all_classified_df = pd.concat(
                [t1_t2_all_classified_df, t1_t2_classified_df], ignore_index=True)
            # t1_t2_all_meta_df = t1_t2_all_meta_df.append(t1_t2_meta_df, ignore_index=True)
            t1_t2_all_meta_df = pd.concat(
                [t1_t2_all_meta_df, t1_t2_meta_df], ignore_index=True)
    ...
    log.info("Writing out first classification file in {}".format(
        second_round_path))
    t1_t2_all_classified_df.to_csv(
        second_round_path+'/{}_classified1.csv'.format(opts.tag), index=False)

    # Tar up the directory
    tar_name = "{}_{}_round1_T1_T2.tar.gz".format(
        os.path.basename(os.path.normpath(opts.main_dir)), opts.tag)
...
if __name__ == "__main__":
    ...
    parser.add_option('--name_tag', type=str,
                      help='Name tag for searching classified files', dest='tag', default='autosave')
    parser.add_option('--username', type=str, help='MGPS/TRAPUM login username; Default is local machine username',
    ...
    parser.add_option('--main_dir', type=str,
                      help='Root directory where all pointing subdirectories are stored', dest='main_dir')
    parser.add_option('--output_dir', type=str,
                      help='Output directory where csvs/tar files will be written out (Default is same as root directory specified)', dest='output_dir')
    parser.add_option('--separate_csvs', type=int,
                      help='Flag for writing out separate csvs for T1/T2 and pulsars (Default:0)', dest='sep_csv', default=0)
    parser.add_option('--second_revision', type=int,
                      help='Flag for writing out a tarball of T1/T2 plots for second revision (Default:1)', dest='second_revision', default=1)
    parser.add_option('--keep_neighbours', type=int,
                      help='Flag for keeping neighbouring beams w.r.t T1 candidates (Default:0)', dest='keep_neighbours', default=0)
    ...
    if opts.sep_csv:
        ...
        log.info("Writing out beams for {} T1/T2 cands in total found in {}".format(
            t1_t2_df.shape[0], opts.main_dir))
        t1_t2_df.to_csv('{}/{}_{}_keep_T1_T2_cands.csv'.format(opts.output_dir,
                        os.path.basename(os.path.normpath(opts.main_dir)), opts.tag), index=False)

    kp_df = write_known_pulsar_beams(opts)
    if opts.sep_csv:
        # Write out known pulsar beam path+names
        log.info("Writing out beams for {} known pulsar cands in total found in {}".format(
            kp_df.drop_duplicates().shape[0], opts.main_dir))
        kp_df.drop_duplicates().to_csv('{}/{}_{}_keep_pulsar_cands.csv'.format(opts.output_dir,
                                                                               os.path.basename(os.path.normpath(opts.main_dir)), opts.tag), index=False)
    ...
    log.info("Writing out beams for all known pulsar and T1/T2 cands (Total: {}) found in {}".format(
        all_beams_df.drop_duplicates().shape[0], opts.main_dir))
    all_beams_df.drop_duplicates().to_csv('{}/{}_{}_keep_beams.csv'.format(opts.output_dir,
                                                                           os.path.basename(os.path.normpath(opts.main_dir)), opts.tag), index=False)

    # Write out a second revision file
    if opts.second_revision:
        log.info(
            "Writing out a tarred file containing T1/T2 candidates for second revision")
