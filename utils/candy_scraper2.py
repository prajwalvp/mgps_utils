...
def get_neighbour_beams(opts, df):
    ...
    for i, row in df.iterrows():
        ...
        for i, beam_num in enumerate(neighbour_beam_list):
            ...
            neighbour_set.loc[i] = [filterbank_path,
                                    opts.username, opts.survey_name + ' T1_CAND n']
...
def write_t1_t2_beams(opts):
    ...
    classified_files = [c for c in glob.glob(
        "{}/*{}*.csv".format(opts.main_dir, opts.tag)) if 'autosave' not in c if 'keep' not in c]
    ...
    for i, classified_file in enumerate(classified_files):
        ...
        if t1_t2_df.empty:
            ...
            t1_t2_beams_df['reason'] = t1_t2_df[t1_t2_df['png'].isin(
                candidate_meta_df['png_path'])]['classification'].to_list()
            t1_t2_beams_df['reason'] = opts.survey_name + \
                ' ' + t1_t2_beams_df['reason'].astype(str)
            ...
            if opts.keep_neighbours == 1:
                if t1_t2_df['classification'].str.contains('T1_CAND').sum():
                    ...
                    t1_beams_df['filterbank_path'] = t1_beams_df['filterbank_path'].apply(
                        lambda x: x.replace(x, os.path.dirname(x)))
                    t1_neighbour_set_df = get_neighbour_beams(
                        opts, t1_beams_df)
                    t1_t2_beams_df = t1_t2_beams_df.append(
                        t1_neighbour_set_df, ignore_index=True)
                else:
...
def write_out_second_revision_tar_file(opts):
    ...
    for i, classified_file in enumerate(classified_files):
        ...
        if t1_t2_df.empty:
            ...
            t1_t2_all_classified_df = t1_t2_all_classified_df.append(
                t1_t2_classified_df, ignore_index=True)
            t1_t2_all_meta_df = t1_t2_all_meta_df.append(
                t1_t2_meta_df, ignore_index=True)
