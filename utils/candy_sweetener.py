...
def untar_file(opts):
    ...
    """
    tar_path = glob.glob("{}/{}/*tar.gz".format(opts.output_path, opts.viewer))[0] 
    my_tar = tarfile.open(tar_path)
    my_tar.extractall("{}/{}".format(opts.output_path,opts.viewer)) # specify which folder to extract to
    my_tar.close()
...
if __name__=="__main__":
    ...
    parser.add_option('--input_zip', type=str, help='Full path to zip file', dest='zip_path')
    parser.add_option('--epoch', type=str, help='Epoch of interest', dest='epoch',default='20210701')
    parser.add_option('--output_dir', type=str, help='Root directory where all pointing subdirectories are stored', dest='output_path')
    parser.add_option('--viewer_name',type=str, help='Input name of viewer as specified in candidates repository',dest='viewer')
    parser.add_option('--viewer_threshold',type=int, help='NUmber of approvals from viewers to select Tier2 candidates',dest='viewer_threshold',default=3)
    ...
    round2_csvs = glob.glob('{}/{}/*round2*.csv'.format(opts.output_path,opts.viewer))
    round1_csv = glob.glob("{}/{}/**/*classified1.csv".format(opts.output_path, opts.viewer))
    ...
    for i,csv in enumerate(round2_csvs):
        df = pd.read_csv(csv)
        classified_data['classification_round2_{}'.format(i+1)] = df['classification'] 
    
    # Apply conditions to select final viewing candidates
    final_cand_dir = "{}/{}_final_phase2_candidates".format(opts.output_path,opts.epoch)
    Path(final_cand_dir).mkdir(parents=True, exist_ok=True)
    log.info("Applying conditions: Looking for at least {} T2 ratings or 1 T1 rating per candidate".format(opts.viewer_threshold))
    for index, row in classified_data.iterrows():
        if (row.str.contains('T2_CAND|TIER2').sum()) >= opts.viewer_threshold or row.str.contains('T1_CAND|TIER1').sum() >=1:
            ...
            png_path = glob.glob('{}/{}/**/{}'.format(opts.output_path,opts.viewer,png_file))[0]
            shutil.copyfile(png_path,final_cand_dir+'/'+os.path.basename(png_path))
